...
pub fn run_server(
    ...
    for tcp_stream in tcp_listener.incoming() {
        if let Ok(stream) = tcp_stream {
            ...
            match handle_connection(stream, &styler, &reader, &drawer, &osm_ids) {
                Ok(_) => {}
                Err(e) => {
...
fn handle_connection(
    ...
    let entities = reader.get_entities_in_tile(&tile, &osm_ids);
    let tile_png_bytes = drawer.draw_tile(&entities, &tile, &styler).unwrap();
