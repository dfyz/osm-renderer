...
fn tile_pixels_benchmark_1(c: &mut Criterion) {
    c.bench_function("create TilePixels, scale = 1", |b| b.iter(|| TilePixels::new(black_box(1))));
}

fn tile_pixels_benchmark_2(c: &mut Criterion) {
    c.bench_function("create TilePixels, scale = 2", |b| b.iter(|| TilePixels::new(black_box(2))));
}
